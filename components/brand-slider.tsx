...
const BrandSlider = () => {
  return (
    <div>
      <h2 className="text-lg tracking-[-4%] font-medium mb-6">Shop by Automobile Brand</h2>
      <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 space-y-[56px]">
        {brands.map((brand, index) => (
          <Link key={index} href={`/brand/${brand.name.toLowerCase()}`} className="flex flex-col text-center mx-auto">
            ...
            <span className="text-sm font-medium tracking-[-3%]">{brand.name}</span>
          </Link>
        ))}
      </div>
    </div>
