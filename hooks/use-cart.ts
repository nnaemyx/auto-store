...
export function useCart() {
  ...
  // Clear cart mutation - client-side implementation
  const clearCart = useMutation({
    mutationFn: async () => {
      // Since there's no API endpoint, we'll just invalidate the query
      // which will cause a refetch, and we'll return an empty cart from our handler
      return { success: true }
    },
    onMutate: async () => {
      // Cancel any outgoing refetches
...
async function removeItemFromCart(itemId: number): Promise<{ success: boolean }> {
  const token = getToken()
  ...
  return apiClient.delete(`/cart/delete-cart/${itemId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
